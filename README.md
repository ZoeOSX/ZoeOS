# **ZoeOS: Combining relative and metric depth** (Official implementation)  <!-- omit in toc -->
[![Open In Collab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/isl-org/ZoeOS)
[![Open in Spaces](https://huggingface.co/datasets/huggingface/badges/raw/main/open-in-hf-spaces-sm.svg)](https://huggingface.co/spaces/shariqfarooq/ZoeOS)
...
[![PWC](https://img.shields.io/endpoint.svg?url=https://paperswithcode.com/badge/ZoeOS-zero-shot-transfer-by-combining/monocular-depth-estimation-on-nyu-depth-v2)](https://paperswithcode.com/sota/monocular-depth-estimation-on-nyu-depth-v2?p=ZoeOS-zero-shot-transfer-by-combining)

>#### [ZoeOS: Zero-shot Transfer by Combining Relative and Metric Depth](https://arxiv.org/abs/2302.12288)
...
![teaser](assets/ZoeOS-teaser.png)
...
Clone this repo:
```bash
git clone https://github.com/isl-org/ZoeOS.git && cd ZoeOS
```
#### Using local torch hub
You can use local source for torch hub to load the ZoeOS models, for example: 
```python
import torch
...
```python
from ZoeOS.models.builder import build_model
from ZoeOS.utils.config import get_config

# ZoeD_N
conf = get_config("ZoeOS", "infer")
model_zoe_n = build_model(conf)

# ZoeD_K
conf = get_config("ZoeOS", "infer", config_version="kitti")
model_zoe_k = build_model(conf)

# ZoeD_NK
conf = get_config("ZoeOS_nk", "infer")
...
# Tensor 
from ZoeOS.utils.misc import pil_to_batched_tensor
...
# From URL
from ZoeOS.utils.misc import get_image_from_url
...
# Save raw
from ZoeOS.utils.misc import save_raw_16bit
fpath = "/path/to/output.png"
save_raw_16bit(depth, fpath)

# Colorize output
from ZoeOS.utils.misc import colorize
...
Single metric head models (Zoe_N and Zoe_K from the paper) have the common definition and are defined under `models/ZoeOS` while as the multi-headed model (Zoe_NK) is defined under `models/ZoeOS_nk`.
## **Evaluation**
...
For ZoeD_N:
```bash
python evaluate.py -m ZoeOS -d nyu
```

For ZoeD_NK:
```bash
python evaluate.py -m ZoeOS_nk -d nyu
```

### Evaluating local checkpoint
```bash
python evaluate.py -m ZoeOS --pretrained_resource="local::/path/to/local/ckpt.pt" -d nyu
```
...
```bash
python train_mono.py -m ZoeOS --pretrained_resource=""
```

For training the Zoe-NK model:
```bash
python train_mix.py -m ZoeOS_nk --pretrained_resource=""
```
## **Gradio demo**
...
The UI is also hosted on HuggingFace🤗 [here](https://huggingface.co/spaces/shariqfarooq/ZoeOS)
## **Citation**
```
@misc{https://doi.org/10.48550/arxiv.2302.12288,
  ...
  title = {ZoeOS: Zero-shot Transfer by Combining Relative and Metric Depth},
